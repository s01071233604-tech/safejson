...
export default function JsonParserPage() {
  ...
  const handleParse = useCallback(() => {
    if (!input.trim()) { setError(null); setParsed(null); return; }
    try { setParsed(JSON.parse(input)); setError(null); } catch (e) {
  ...
  const handleSample = useCallback(() => {
    ...
    setInput(t); setParsed(JSON.parse(SAMPLE)); setError(null);
  }, []);

  const typeInfo = useMemo(() => parsed ? getTypeInfo(parsed) : [], [parsed]);
            ...
            <div className="flex-1 min-h-[360px] overflow-auto">
              {error ? (
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2 border border-zinc-800 rounded-lg bg-zinc-900/30"><JsonTreeView data={parsed} /></div>
                </div>
              ) : (
